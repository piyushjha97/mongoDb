...
def benchmark_skiplimit(count, page_size):
    ...
    while items_fetched < count:  # Stop when all items are fetched
     ...
     #   for _ in range(run_count):
        ...
        # Debug statement to trace the number of items fetched
       # print(f"Page number: {page_number}, Items fetched this page: {len(data)}, Total items fetched so far: {items_fetched}")

    #print(f"Items fetched: {items_fetched}, Expected: {count}")  # Debug statement
...
def print_stats(times, count, page_size, approach, file):
    """Print statistical metrics."""
    s = stats.all(times)
    output = "\"{}\",{},{},{},{},{},{}".format(approach, count, page_size, s['mean'], s['p99'], s['p95'], s['p50'])
    print(output)
    file.write(output + "\n")

def bench(count, page_size, file):
    """Run benchmarks for both pagination strategies."""
    approach = 'skiplimit'
    times = benchmark_skiplimit(count, page_size)
    print_stats(times, count, page_size, approach,file)
...
if __name__ == '__main__':
    ...
    print(header)
    with open('/Users/piyushjha97/Desktop/mongoDb/res.csv', 'w') as file:
        file.write(header + "\n")
